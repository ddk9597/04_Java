...
public class SetService {
	 ...
	 * Hashset how to use
	 * 
	 * 	사용 조건 1. : 객체에 equals()   가 오버라이딩 되어있어야 함
	 *  사용 조건 2. : 객체에 hashCode() 가 오버라이딩 되어있어야 함
	 * 
	 * 	String, integer등 자바에서 제공하는 객체는 모두 오버라이딩 되어있는 상태!!
	 * 
	 */
	public void method1() {
		
		// HashSet 객체 생성
		Set<String> set = new LinkedHashSet<String>(); // 다형성 - 업캐스팅 적용됨
		...
		// 전달받은 e를 Set 에서 제거 - 제거 성공 true, 없으면 false
		...
		// 5. void clear() : Set 에 저장된 내용 모두 삭제
		set.clear();
	...
	public void method2() {
		...
		// Iterator Set.Iteraotr : 현재 Set을 순차 접근할 수 있는 Iterator 객체 반환
	...
	public void method3() {
		...
		for(Person p : personSet) {
			System.out.println(p);// 25세 홍길동이 한번만 출력된다
		}
	...
	/**
	 * 로또 번호 생성기
	 * 금액을 입력 받아(1천원 단위) 1000원당 1회씩 번호를 생성하여
	 * List 에 저장 한 후 생성 종료 시 한번에 출력
	 * 
	 * ex)
	 * 금액 입력 : 3000
	 * 
	 * 1회 : [12,34,37,12,45,41]
	 * 2회 : [1,15,39,42,10,12]
	...
	public void evolvedottoNumberGenerator() {
		...
		int chance = inputMoney / 1000 ;
		
		Random random = new Random();
		
		List<List<Integer>> allLotto = new ArrayList<List<Integer>>();
		
		// for(int i=0, end = chance ; i < end ; i ++)
		// for문 초기식에 여러 식 작성 가능
